#LP relaxation of Unaligned Version of Problem
set User;
set Item;
...
var x {u in User, p in 1..1, i in Item} >=0, <=1;
var y {u in User, v in User, p in 1..1, i in Item} >=0, <=1;
var z {u in User, v in User, i in Item} >=0, <=1;
...
obj_person = sum {u in User , i in Item} (preference[i, u] * (sum{p in 1..1} x[u,p,i]));

subject to obj_social_aligned_definition:
obj_social_aligned = sum {u in User, v in User, i in Item} (social_aware_weight[u,v,i] * (sum {p in 1..1} y[u,v,p,i]));
...
subject to one_item_each_position {p in 1..1, u in User}:
sum{i in Item} x[u,p,i]==1;

subject to once_an_item {i in Item, u in User}:
sum{p in 1..1} x[u,p,i] <= 1;

subject to x_larger_then_y_1 {u in User, v in User, p in 1..1, i in Item}:
y[u,v,p,i] <= x[u,p,i];

subject to x_larger_then_y_2 {u in User, v in User, p in 1..1, i in Item}:
y[u,v,p,i] <= x[v,p,i];

subject to x_larger_than_z_1 {u in User, v in User, i in Item}:
z[u,v,i] <= sum{p in 1..1} x[u,p,i];

subject to x_larger_than_z_2 {u in User, v in User, i in Item}:
z[u,v,i] <= sum{p in 1..1} x[v,p,i];

subject to if_y_then_no_z {u in User, v in User, i in Item}:
sum {p in 1..1} y[u,v,p,i] + z[u,v,i] <= 1;
